...
namespace MedFarLab.Api.Controllers
{
    ...
    public class SystemController : ControllerBase
    {
        ...
        public SystemController(IActionDispatcher dispatcher)
        {
        ...
        public async Task<IActionResult> Get(int actionCode, [FromQuery] string? payload = null, [FromServices] MedfarLabs.Core.Domain.Interfaces.Security.IUserContext userContext = null)
        {
            ...
            var result = await _dispatcher.DispatchAsync((AppModule)12, actionCode, jsonPayload, HttpContext.TraceIdentifier, branchId);
            return Ok(result);
        }
        ...
        {
            long branchId = Request.Headers.TryGetValue("X-Branch-Id", out var bStr) && long.TryParse(bStr, out var b) ? b : 0;
            var result = await _dispatcher.DispatchAsync((AppModule)12, actionCode, payload, HttpContext.TraceIdentifier, branchId);
            return Ok(result);
        }
    }
}
