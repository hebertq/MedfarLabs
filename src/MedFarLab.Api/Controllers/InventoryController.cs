...
namespace MedFarLab.Api.Controllers
{
    ...
    public class InventoryController : ControllerBase
    {
        ...
        public InventoryController(IActionDispatcher dispatcher)
        {
        ...
        public async Task<IActionResult> Get(int actionCode, [FromQuery] string? payload = null)
        {
            ...
            var result = await _dispatcher.DispatchAsync(AppModule.Inventory, actionCode, jsonPayload, HttpContext.TraceIdentifier, branchId);
            return Ok(result);
        }
        ...
        {
            long branchId = Request.Headers.TryGetValue("X-Branch-Id", out var bStr) && long.TryParse(bStr, out var b) ? b : 0;
            var result = await _dispatcher.DispatchAsync(AppModule.Inventory, actionCode, payload, HttpContext.TraceIdentifier, branchId);
            return Ok(result);
        }
    }
}
