...
namespace MedFarLab.Api.Controllers
{
    ...
    public class CareController : ControllerBase
    {
        ...
        public CareController(IActionDispatcher dispatcher)
        {
        ...
        public async Task<IActionResult> Get(int actionCode, [FromQuery] string? payload = null, [FromServices] MedfarLabs.Core.Domain.Interfaces.Security.IUserContext userContext = null)
        {
            ...
            var result = await _dispatcher.DispatchAsync(AppModule.Care, actionCode, jsonPayload, HttpContext.TraceIdentifier, branchId);
            return Ok(result);
        }
        ...
        {
            Console.WriteLine($"[API_REQUEST_BODY] {payload.GetRawText()}");
            long branchId = Request.Headers.TryGetValue("X-Branch-Id", out var bStr) && long.TryParse(bStr, out var b) ? b : 0;
            var result = await _dispatcher.DispatchAsync(AppModule.Care, actionCode, payload, HttpContext.TraceIdentifier, branchId);
            return Ok(result);
        }
    }
}
