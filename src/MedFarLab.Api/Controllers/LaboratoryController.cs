...
namespace MedFarLab.Api.Controllers
{
    ...
    public class LaboratoryController : ControllerBase
    {
        ...
        public LaboratoryController(IActionDispatcher dispatcher)
        {
        ...
        public async Task<IActionResult> Get(int actionCode, [FromQuery] string? payload = null)
        {
            ...
            var result = await _dispatcher.DispatchAsync(AppModule.Laboratory, actionCode, jsonPayload, HttpContext.TraceIdentifier, branchId);
            return Ok(result);
        }
        ...
        {
            long branchId = Request.Headers.TryGetValue("X-Branch-Id", out var bStr) && long.TryParse(bStr, out var b) ? b : 0;
            var result = await _dispatcher.DispatchAsync(AppModule.Laboratory, actionCode, payload, HttpContext.TraceIdentifier, branchId);
            return Ok(result);
        }
    }
}
